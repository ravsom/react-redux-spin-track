import {
	...
	LOGOUT_USER, UPDATE_USER_EMAIL, FACEBOOK_SIGNIN_USER
} from '../actions/users';
...
export default function(state = INITIAL_STATE, action) {
	...
	switch (action.type) {
		...
		case FACEBOOK_SIGNIN_USER:
			console.log("Facebook signin user.");
			return {...state, user: null, status: 'signup', error: null, loading: false};
		case SIGNUP_USER:// sign up user, set loading = true and status = signup
			console.log("reducer_user: SIGNUP_USER");
		...
		case SIGNUP_USER_SUCCESS://return user, status = authenticated and make loading = false
			console.log("reducer_user: SIGNUP_USER_SUCCESS");
			return {...state, user: action.payload.data.user, status: 'authenticated', error: null, loading: false}; //<-- authenticated
